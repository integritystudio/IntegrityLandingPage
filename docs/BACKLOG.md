...
| [CR29](#cr29) | P1 | ✅ **fix LIVE 2026-08-03** — step 3 (inert unbind) left | **The HMAC keyless-downgrade forgery path is closed in production.** Sender fail-closed (`d6764d99`) + receiver requires `x-key-id` (`e964852d`) deployed via manual `deploy:prd`. Verified with curl + controls: keyless `SHARED_SECRET` → **401** (was **200**); `v2` positive control passes signature; legit `/signin`→`/send` → `ok:true`. Remaining: step 3, unbind the now-**inert** `SHARED_SECRET` (hygiene), gated on `auth.key_unresolved{miss:"missing_key_id"}` staying 0 over a traffic cycle. ⚠️ Sender fix is on branch `fix/active-subscription-id` — merge to make step 1 durable (security fix is receiver-side, on toolkit `main`, durable) |
...
**What remains: step 3 only** — unbind `SHARED_SECRET`. It is now **inert** (the receiver resolves solely through `SIGNING_KEYS`; neither side reads it), so this is hygiene, not the fix. Gated on watching `auth.key_unresolved{miss:"missing_key_id"}` stay at zero in deployed traffic over a full cycle — that confirms no unknown caller was relying on the keyless path (the only known caller, sender-worker, sends `v2`). Leaving the inert secret bound during that window costs nothing; do the unbind after. ~~Open — every code step is done (0, 0b, 1, 2); what remains is a deploy and then step 3.~~ Step 0 (2026-07-31) found one automated caller signing without a key id (the CI e2e job) and that job has been **removed**, so `sender-worker` — signing `v2` — is the sole automated caller and nothing depends on the keyless fallback. Step 0b's instrumentation is committed **unpushed** (2026-08-01, `8fcae0b`); steps 1 and 2 followed on 2026-08-02, also unpushed, in both repos. **Order, with what remains:** ~~0b instrument the receiver~~ ✅ → ~~step 1 sender fail-closed~~ ✅ → ~~step 2 receiver requires the header~~ ✅ → **deploy** ← the whole remaining risk → step 3 unbind.
