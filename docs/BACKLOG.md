...
0. ✅ **Done 2026-07-31 — and it found a blocker.** The audit ran against `observability-toolkit` at `~/.claude/mcp-servers/observability-toolkit`. Both halves are answered below; **step 2 cannot ship until the e2e finding is resolved**, so this step's outcome is a hard prerequisite, not a clearance.
   ...
   **The suite cannot simply add the header: `ACTIVE_KEY_ID` and `SIGNING_KEYS` are both UNSET in Doppler `dev`** (verified by fingerprint; `prd` has both). And because the dev config points at the *production* receiver, provisioning them in `dev` would mean copying a production signing key into `dev` — re-creating exactly the [[CR11]] sharing problem this work is meant to end. **So a dev receiver ([[CR02]] item 5) is a prerequisite for step 2, not an unrelated nicety.** Alternatives, in preference order: stand up a dev receiver and point `dev` at it; or move this suite to `--config prd` on the same reasoning as `sender-worker`'s `test:live`; or delete tests 2–4 and accept the coverage loss.

   **b. The receiver does not log the resolved key id, so the "watch a full traffic cycle" evidence is not obtainable today.** `services/api-provisioning-receiver/src/index.ts` reads `keyId` at line 95, passes it to `resolveSigningKey` at 96, and **never records it**. There is no auth event on the success path at all, and `captureAuditEvent({ event: "auth.invalid_signature" })` fires for *both* "no secret resolved" and "signature mismatch" with no discriminator — so even failures cannot be attributed to a missing header. Adding that instrumentation is the first code change of this item, ahead of step 1.

   Two smaller findings. `resolveSigningKey` in the real receiver (`src/utils.ts:50–61`) is behaviourally **identical** to this repo's stub, so the stub is a faithful double and the "must mirror" note below holds. And `src/utils.test.ts:16` asserts *"returns SHARED_SECRET when keyId is undefined"* — both repos' unit tests pin the behaviour step 2 removes, so they change in the same commit. Out of scope, confirmed not affected: `obtool-ingest`'s evaluations endpoint is a **separate** HMAC scheme (`INJECT_HMAC_SECRET`, body-only, `sha256=` prefix) and does not touch `SHARED_SECRET`.
1. **Sender fails closed** instead of downgrading: when `ACTIVE_KEY_ID`/`SIGNING_KEYS` are present but unusable, error rather than signing with `SHARED_SECRET`. Cheap, local, and safe to ship alone.
...
**Files to touch:**
- `api-provisioning-receiver` (`observability-toolkit` repo, `services/api-provisioning-receiver/`) — the real change (step 2)
...
- `workers/sender-worker/src/utils.ts` — `resolveOutboundSigningKey` fail-closed (step 1)
...
**Status:** Open — **step 0 done 2026-07-31, steps 1–3 blocked on what it found.** Diagnosis proven with controls; no code changed. The caller audit is no longer the blocker: it is complete, and it converted the open question into two concrete prerequisites. **Revised order:** instrument the receiver to record the resolved key id (0b) → resolve the CI e2e caller, which needs a dev receiver ([[CR02]] item 5) → step 1 sender fail-closed → step 2 receiver requires the header → step 3 unbind. Steps 1 and 3 are unchanged and still safe in that position; only step 2 acquired a dependency. Related: [[CR11]] row #7 (how it was found, and why provisioning `SIGNING_KEYS` into `dev` is not the fix), [[CR02]] item 5 (the dev receiver, now a prerequisite), [[CR01]] step 3 (the rotation this makes incomplete), [[W05]] (runbook — rewritten 2026-07-31), [[CR14]] (the receiver's retained versions publish whatever it binds, so step 3 also shrinks that exposure).
