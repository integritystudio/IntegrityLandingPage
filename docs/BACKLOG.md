...
3. ⚠️ **In progress (2026-07-29). Per-family state, verified by probe:**
   ...
   - ⚠️ **Auth0**: `AUTH0_CLI_SECRET` (M2M client → Management API, used by `/signup`) rotated 2026-07-29, validated via a `client_credentials` grant against the tenant, and re-bound to production `sender-worker` (healthy after). Doppler `dev` still holds the previous value. **`AUTH0_CLIENT_SECRET` (ROPC client): the value now in Doppler (identical in `dev` and `prd`) fails client authentication, while live `/signin` — running on the previously-bound secret — still returns a JWT. The old secret was therefore never invalidated at Auth0, and Doppler holds a wrong value (possibly copied from a different Auth0 application). Production is unaffected; do not re-bind until the correct secret is in Doppler.**
