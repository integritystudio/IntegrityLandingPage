...
**Priority:** P3 | **Source:** session 2026-08-06, verifying [[CR12]]'s HMAC secret bind and [[CR18]]'s deleted key
...
**Scope:**
1. Document the gotcha in this repo's `CLAUDE.md` (or wherever Doppler CLI conventions are recorded) so the next person — human or agent — doesn't reach for `secrets get NAME --plain` to check existence.
2. Grep `scripts/*.sh` and any CI workflow for `secrets get .* --plain` patterns that assume a clean miss (none currently do — `check-env-isolation.sh`'s `digest()` function backgrounds stderr with `2>/dev/null` and only reads stdout, which is why it was unaffected — but a future script might not).
3. Optional: a thin wrapper (`doppler-get-safe` or similar) that checks existence via `--only-names` before ever calling `--plain`, for use in any future verification script or agent-facing runbook step.

**Status:** Open — no live risk (the exposure was local tool output in one session, not committed, not sent externally), but worth a documentation fix so it isn't rediscovered the same way.
...
**Status:** Open — 6 of 7 listed values fixed. `CLOUDFLARE_D1_TOKEN` remains and is now **characterised rather than merely flagged**: one account-wide D1 **write** token shared by both configs, whose per-database scoping is **impossible**, so the item's remaining decision is accept-and-document vs separate accounts — not a minting task. ✅ The one cheap improvement available **is now done** — `obtool-d1-readonly` carries the daily signals check, `prd`-only and write-less — but it narrows the *scheduled path*, not the item: `CLOUDFLARE_D1_TOKEN` is still shared, unexpiring and account-wide-write for the runbook's rewind. The generalisable half is **measured rather than argued** (170 shared names, 117 identical, 4 illegitimate, 2 of them live) but still not implemented: the detector compares a hand-maintained list, and the four were found by an ad-hoc script, not by `check:env-isolation`.
...
| [CR13](#cr13) | P2 | ⚠️ **decided 2026-08-08 — blocked on a registrar action** | **Option C: the gateway gets `api.integritystudio.dev`** (owner decision), which closes steps 3–5 and **supersedes [[CR31]]'s option-B path-split** — [`api-routing.md`](api-routing.md) still recommends B and now disagrees with the entry. The name is already the gateway's Auth0 audience/resource server (`69c4e28bf801eab9e683c85a`), though an audience is opaque and was never obliged to resolve. 🔴 **`integritystudio.dev` is not a Cloudflare zone** — it delegates to `*.ns.porkbun.com`, so no Workers route or Custom Domain can attach; the remaining work is an NS change at the registrar, not config. Migration is a delegation change, not a transfer; DNSSEC is off and there is no MX/TXT/CAA, so both usual blockers are clear. The apex is ruled out on evidence — it serves the GitHub Pages dashboard, 200. ⚠️ Adding `routes` before the zone exists breaks `deploy:prd`; the config change is last. Step 1 remains done and **proven by a real deploy 2026-07-30** |
