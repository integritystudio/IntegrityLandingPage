...
3. ⚠️ **In progress (2026-07-29). Per-family state, verified by probe:**
   ...
   - ⚠️ **In-use `sb_secret_` service keys are unrotated**: the workers still bind the same `SUPABASE_SERVICE_ROLE_KEY` (and Doppler also holds `SUPABASE_SERVICE_KEY`) from before this session. If those keys predate the April-1 `doppler.json` snapshot they are leaked material that still authenticates (probe 200). Cheapest close-out: bind the new `prd SUPABASE_PROVISIONING_KEY` value as `SUPABASE_SERVICE_ROLE_KEY` on `api-gateway`, `sender-worker`, `stripe-webhook`, and the receiver, verify, then revoke the old `sb_secret_` keys in Supabase.
