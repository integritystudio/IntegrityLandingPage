...
5. 🔴 **`SUPABASE_JWT_SECRET` may not exist in the old sense on a new project.** Production's signing keys are HS256 `previously_used` + **ES256 `in_use`**, i.e. it has moved to asymmetric keys, which is the default for new projects. Our verifier is **HS256 with a shared secret** (`verifyJwt` in `workers/lib/auth.ts`). A new dev project would likely have no usable HS256 secret to bind, so `api-gateway`/`bootstrap-worker` JWT verification could not be exercised there at all. Decide this *before* creating the project: either keep a legacy HS256 secret on the new project, or move the verifier to JWKS/ES256 — which the production side will need eventually anyway.
