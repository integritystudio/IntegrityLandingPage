...
0. **Confirm nothing legitimate still signs without a key id**, before removing the path. Two parts: audit the caller set in `observability-toolkit` (this repo's `sender-worker` is fine — it has `v2`; `sender-worker-dev` has zero secrets bound so it cannot sign at all, re-verified live 2026-07-31: `wrangler secret list --name sender-worker-dev` → `[]`; that is not the whole set), and get *positive* evidence from the receiver that no absent-key-id request arrives. **Whether the production receiver logs the resolved key id is unknown from here** — `observability-toolkit` is not checked out on this machine, so it was not read; the local stub (`workers/receiver-worker/src/index.ts`) has no `console.*` call at all, which is weak evidence the real one is similar. Establish that first: if resolved-key-id is not logged, log it and watch a full traffic cycle before proceeding. An unbind based on "we think nothing uses it" is the outage this ordering exists to prevent.
...
**Status:** Open. Diagnosis complete and proven with controls; no code changed. Blocked on the cross-repo caller audit (step 0), which is a prerequisite rather than a decision. Related: [[CR11]] row #7 (how it was found), [[CR01]] step 3 (the rotation this makes incomplete), [[W05]] (the runbook that needs the caveat), [[CR14]] (the receiver's retained versions publish whatever it binds, so step 3 also shrinks that exposure).
