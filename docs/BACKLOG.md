...
**Detector:** `npm run check:env-isolation` — compares credential hashes between the two configs, prints no secret material, exits non-zero while they are shared. **Currently fails 6 of 13** (was 10 of 10 before the 2026-07-29 cleanup; the row count grew when Stripe coverage was added). A green run is the definition of done for this item — but see the 2026-07-29 update below: 1 of the 6 is not reachable by API at all.
...
**Sequenced target:** ~~#7 alone gets 7→6 with zero decisions~~ — **done, now at 6.** Adding the Auth0 dev connection + dev clients gets 6→3. Adding a dev Supabase project gets 3→1. The last row needs one Dashboard visit to create a second Auth0 tenant, after which the dev client/connection work should be redone inside it.
