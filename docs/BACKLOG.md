...
**Status:** Open — **step 0 done 2026-07-31, and its blocker cleared the same day; steps 1–3 are unblocked.** Diagnosis proven with controls; no production code changed yet. Step 0 found one automated caller signing without a key id (the CI e2e job) and that job has been **removed**, so `sender-worker` — signing `v2` — is the sole automated caller and nothing now depends on the keyless fallback. **Revised order:** instrument the receiver to record the resolved key id (0b, still first — the positive evidence is otherwise unobtainable) → step 1 sender fail-closed → step 2 receiver requires the header, remembering it **auto-deploys on merge to `main`** → step 3 unbind. The dev receiver ([[CR02]] item 5) is **no longer on this critical path**; it is needed to *restore* the e2e job, not to ship step 2. Related: [[CR11]] row #7 (how it was found, and why provisioning `SIGNING_KEYS` into `dev` is not the fix), [[CR02]] item 5 (the dev receiver, now a prerequisite), [[CR01]] step 3 (the rotation this makes incomplete), [[W05]] (runbook — rewritten 2026-07-31), [[CR14]] (the receiver's retained versions publish whatever it binds, so step 3 also shrinks that exposure).
