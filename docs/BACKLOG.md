...
**Still open, deliberately:** the Dart model types `currentMinuteRemaining` as a non-nullable `int` defaulting to `0`, while the server always sends `null` for it (the value is owned by the quota Durable Object). The client therefore renders `0` for a value that is unknown — the same "unavailable rendered as zero" failure as item 5, one layer up. The fix is for the client to read it from `/v1/orgs/:id/quota/status`, which needs a Flutter release.
