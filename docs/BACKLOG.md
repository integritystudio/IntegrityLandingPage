...
**Last Updated:** 2026-09-22 (UA07 implemented `dbe6749f` and verified in production; UA08 filed from what the verification showed). Previous: 2026-09-21 (UA07 filed), 2026-09-20 (UA01–UA06 filed; UA01 implemented `8edf955`).
...
| [CR29](#cr29) | P1 | ✅ **RESOLVED 2026-08-03** | **The HMAC keyless-downgrade forgery path is closed and the legacy credential eliminated.** Steps 1+2 deployed (sender fail-closed, receiver requires `x-key-id`); step 3 done — `SHARED_SECRET` made optional in the receiver `Env`, unbound from both workers, and deleted from **both** Doppler configs (`prd` 2026-08-03, `dev` 2026-09-24) plus `KEY_ROTATION_DATES` in both (`prd` `{v2}`, `dev` `{dev1}`). Verified in prod: keyless `/inbox` → **401** (was 200), `v2` passes signature, `/signin`→`/send` → `ok:true`. ⚠️ Sender fix on branch `fix/active-subscription-id` — merge to make step 1 durable (security fix is receiver-side, on toolkit `main`) |
...
### UA03: Doppler's `KV_NAMESPACE_ID` is the dashboard namespace, not the AUTH namespace the key functions write
...
### UA06: Org and identity hygiene — four small inconsistencies that break joins the rest of the system assumes
