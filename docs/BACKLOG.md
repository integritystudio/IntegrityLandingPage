...
**Status:** Open — **steps 1, 2, 4, and 5 are done.** Step 1: instrumentation deployed and emitting on all four production Workers (2026-07-30). Step 2: signals defined and executable (2026-07-31, see above). Step 4: daily scheduled alert job added 2026-08-08 (`.github/workflows/worker-signals.yml` — GitHub job-failure email is the channel; Supabase creds enable SIGNAL 5 dead-letter depth) — ⚠️ **added, not armed: `schedule` workflows run from the default branch only, so it cannot fire until this branch merges to `main`, and no scheduled run has ever executed.** The check itself is verified live (all five signals, exit 0, `stripe-webhook` 1.00 subreqs/req); the *scheduling and notification* half is unproven. See the red note under [[CR20]]. Step 5: monitoring runbook added to `docs/api-provisioning.md` 2026-08-08. **Remaining: step 3 only** (dashboard — **still blocked on `obtool-ingest` being repaired**, see the note there), plus confirming step 4 actually fires post-merge.
