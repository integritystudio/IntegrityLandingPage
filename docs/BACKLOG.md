...
3. ⚠️ **In progress (2026-07-29). Per-family state, verified by probe:**
   ...
   - 🔴 **New finding — two live `rk_live_` Stripe keys on the production account.** `prd STRIPE_SECRET_KEY` (ends `aHZC`) and `prd STRIPE_API_KEY` (ends `B6I8`) are *different* restricted keys, and `GET /v1/account` returns 200 for both against `acct_1SN2e7AwEfePbhfk`. Worker code reads **only** `STRIPE_SECRET_KEY` (4 uses; `STRIPE_API_KEY` appears solely in docs and a sandbox `stripe-webhook-dev` binding), so `…B6I8` is an unused live production credential. Revoke it in the Dashboard — Stripe exposes no key-management API — and clear the slot afterwards, not before, since the last-4 is how the Dashboard identifies it. This is in addition to the pre-rotation Stripe key still awaiting revocation.
...
- ⚠️ **Fallout: `cd workers/sender-worker && npm run test:live` no longer runs, by design.** It resolves credentials with `doppler run --config dev` and exchanges `AUTH0_CLIENT_ID`/`AUTH0_CLIENT_SECRET` for a **Management API** token. Those slots now hold `integrity-dev-ropc`, whose grants are `password`/`password-realm`/`refresh_token` only, so the exchange returns `403 unauthorized_client — Grant type 'client_credentials' not allowed for the client` and the suite fails in `beforeAll` (12 tests skipped). **This is the intended outcome, not a regression to undo:** a dev credential should not hold Management API power over the tenant that owns all 96 production users, and re-adding `client_credentials` plus a Management grant would hand it back. Note also that the suite asserts on `AUTHO_ACCESS_TOKEN_API_KEY`, which was cleared because it held a 241-day-expired token from a *different* tenant, so it had failing assertions regardless. Two honest ways forward, both needing a decision: point `test:live` at `--config prd`, acknowledging that it creates and deletes a real user in the production tenant (which it always did, since `dev` and `prd` were identical before), or wait for the second Auth0 tenant and re-target the suite there. Worth noting what this exposes: production's `My App` carries `client_credentials` *and* Management API authorisation purely so this test suite can mint admin tokens — see [[CR25]] item 8.
...
5. ✅ **RESOLVED IN CODE 2026-07-29 — the verifier now supports JWKS/ES256.** The problem was real: production's signing keys are HS256 `previously_used` + **ES256 `in_use`**, so the project already issues asymmetric tokens, while `verifyJwt` only did HS256 against a shared secret — meaning a new dev project (ES256 by default, no legacy secret) could not have exercised JWT verification at all, and production HS256 verification is on borrowed time. **`workers/lib/auth.ts` now verifies ES256 and RS256 against the project's published key set**, keeping HS256 as a fallback so tokens minted before the migration still verify until they expire. Details worth knowing:
   ...
   - **Verified:** 16 new unit tests (locally generated P-256 key pair — no network), plus a live check that the real project's published key (`kid b91503ee-…`, ES256) imports under exactly these WebCrypto parameters and **rejects a token forged with a different key**. Full sweep: **1,059 worker tests passing**, zero TypeScript errors.
