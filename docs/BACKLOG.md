...
| Auth0 dev tenant (step 2) | **free** — additional development/staging tenants link to the same subscription with usage aggregated at the subscription level, so a tenant at ~0 MAU adds no charge on the free plan | ❌ no endpoint exists; `AUTH0_CLI_*` lacks `create:tenants` | one Dashboard visit |
...
2. **Create the Auth0 dev tenant** and a matching M2M + ROPC application pair. Free (see *Blockers and cost*), but **not scriptable at any price**: the `AUTH0_CLI_*` M2M app is scoped to the existing tenant's Management API — **confirmed 2026-07-29 by probe**, the token lacks `create:tenants` and no tenant-creation endpoint exists. One Dashboard visit. The `dev-users` connection + dev clients built inside the *production* tenant on 2026-07-29 are the API-scriptable partial (detailed in the 2026-07-29 update below); they are leak-surface reduction, and **step 2 retires them** once a real tenant exists.
