...
**~~None of the four locatable configs sets `preview_urls`~~ **— fixed for all three `observability-toolkit` Workers 2026-08-03.** `api-provisioning-receiver`, `obtool-ingest` and `obtool-api` now each set `preview_urls = false` **in config**, so this is durable rather than an API-level flip that the next deploy would undo — which is exactly what the struck sentence warned about. That warning was right about the mechanism and is now moot for these three. `tcad-api` holds 7 secrets but has `workers.dev` disabled outright, so it is not exposed `tcad-api` holds 7 secrets but has `workers.dev` disabled outright, so it is not exposed.
