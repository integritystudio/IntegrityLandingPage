...
| [CR29](#cr29) | P1 | 🔴 open | **The HMAC key rotation is a no-op.** `SIGNING_KEYS`/`v2` is provisioned and works, but the receiver resolves an **absent** `x-key-id` to the legacy `SHARED_SECRET` — a credential with no key id and therefore no rotation handle. Proven against production `/inbox` with controls: `v2` + key id → 200, `SHARED_SECRET` + no key id → **200**, garbage → 401. So removing a key from `SIGNING_KEYS` revokes nothing, [[CR01]]'s HMAC rotation is incomplete, and anything holding `SHARED_SECRET` (incl. Doppler `dev`) can forge provisioning events. Sender has the mirror hazard — a typo'd `ACTIVE_KEY_ID` silently downgrades. **Step 0's caller audit is done (2026-07-31) and its blocker is cleared:** it found `observability-toolkit`'s CI e2e suite signing `/inbox` with `SHARED_SECRET` and no key id against the **production** receiver on every publish, and since it could not just add the header (`SIGNING_KEYS`/`ACTIVE_KEY_ID` unset in Doppler `dev`, and provisioning them there re-creates [[CR11]]) **that job has been removed**. `sender-worker` (signs `v2`) is now the sole automated caller, so steps 1–3 are unblocked and the dev receiver is off the critical path. **Step 0b's instrumentation is done 2026-08-01** (`observability-toolkit` `8fcae0b`, committed **not pushed** — the receiver **auto-deploys on merge to `main`**): the resolved key id and source are now recorded, with `auth.verified_legacy_key` as the metric that gates step 3. **Step 1 is done 2026-08-02** — the sender now returns 500 `SIGNING_KEY_UNRESOLVED` and forwards nothing when `ACTIVE_KEY_ID` cannot be resolved, instead of signing with `SHARED_SECRET`; unshipped, since `sender-worker` deploys on merge to `main`. Next: **step 2**, the receiver requiring `x-key-id` |
...
**Sequenced target:** ~~#7 gets 7→6~~ and ~~the Auth0 dev connection + dev clients get 6→3~~ — both done 2026-07-29, reaching 3 of 13 that day. ⚠️ **Not the current state: 5 of 13 as of 2026-07-31** — #7 regressed and the `SUPABASE_SERVICE_ROLE_KEY` row is now a phantom failure against a deleted slot (see the 2026-07-31 update above). The Auth0 half of this has held. What remains: a dev Supabase project takes 3→1 (spend/quota decision, no dry-run for `POST /v1/projects`), and the final row needs one Dashboard visit to create a second Auth0 tenant — after which the connection/client work above should be redone inside it, and the `dev-users` connection in the production tenant retired.
...
**The sender has the mirror-image hazard.** `resolveOutboundSigningKey` (`workers/sender-worker/src/utils.ts`) falls back to `SHARED_SECRET` with no key id on **four** conditions — `ACTIVE_KEY_ID` unset, `SIGNING_KEYS` unset, `SIGNING_KEYS` malformed JSON, or `ACTIVE_KEY_ID` naming a key that is not in the map. Only a `console.warn`/`console.error` marks the last two. So a typo in `ACTIVE_KEY_ID` silently drops production back to the legacy key and keeps working, which is precisely the failure a rotation mechanism exists to make loud.
...
1. ✅ **Done 2026-08-02 — sender fails closed** instead of downgrading. `resolveOutboundSigningKey` now returns a discriminated union mirroring the receiver's `SigningKeyResolution`: `{ secret, keyId }` on success, or `{ secret: null, miss }` where `miss` is `signing_keys_unset` | `signing_keys_malformed` | `unknown_active_key_id`. `forwardToReceiver` narrows on `secret === null` and returns **500 `SIGNING_KEY_UNRESOLVED`** without calling the receiver. Four things worth knowing before touching it:
   - **The trigger is `ACTIVE_KEY_ID` *set* and unresolvable, not the absence of rotation vars.** `ACTIVE_KEY_ID` unset stays on the legacy path deliberately — `wrangler.toml` documents it as the way to stage `SIGNING_KEYS` before activating it, and it is still valid configuration until step 3. Failing closed on every fallback would break that staging state and the shipped legacy config with it.
...
2. **Receiver requires `x-key-id`** and resolves solely through `SIGNING_KEYS` — absent header becomes a 401, same as `""` already is. **Cross-repo.** This is the step that closes the forgery path.
3. **Unbind `SHARED_SECRET`** from both Workers once steps 0–2 hold. Until then it stays bound, because a binding that is no longer read is harmless and an unbinding done early is an outage.
...
**One apparent blocker to step 2, already retired — but it will be re-created unless a comment changes with the code.** `workers/sender-worker/wrangler.toml:104` states that `ACTIVE_KEY_ID` is *"deliberately UNSET in the dev config: `sender-worker-dev` binds `RECEIVER` to the PRODUCTION receiver … so it must keep taking the `SHARED_SECRET` fallback path."* Read at face value that is a legitimate caller signing without a key id, and therefore a reason step 2 cannot ship. It is not, for two independent reasons. **`sender-worker-dev` has no secrets bound at all** (`wrangler secret list --name sender-worker-dev` → `[]`, 2026-07-31), so it can neither sign with `v2` *nor* fall back — the documented dependency is inert. And the configuration that would activate it is one this repo forbids: pushing the `dev` Doppler secrets into the dev Workers "would create a second production-capable worker rather than a dev environment" (CLAUDE.md; [[CR11]]). So the fallback is not load-bearing for dev today and must not become so. **Delete that comment in the same change as step 2** — left in place it is a standing instruction to restore the very path being removed, and the next operator reading it will reasonably conclude the removal was a mistake. The real fix for dev is a dev receiver ([[CR02]] item 5), not a shared legacy credential.
...
- `workers/sender-worker/wrangler.toml:104` — the `ACTIVE_KEY_ID` comment asserting dev "must keep taking the `SHARED_SECRET` fallback path" (see the paragraph above); annotated 2026-07-31, delete with step 2
...
**Status:** Open — **steps 0, 0b, and 1 are done; step 2 is next and it is the one that closes the forgery path.** Step 0 (2026-07-31) found one automated caller signing without a key id (the CI e2e job) and that job has been **removed**, so `sender-worker` — signing `v2` — is the sole automated caller and nothing now depends on the keyless fallback. Step 0b's instrumentation is committed **unpushed** (2026-08-01, `8fcae0b`); step 1 is committed here (2026-08-02) and **not deployed** — `sender-worker` is CI-deployed on merge to `main`, and this branch is far ahead of it. **Order, with what remains:** ~~0b instrument the receiver~~ ✅ (unpushed, and **pushing it deploys the receiver**) → ~~step 1 sender fail-closed~~ ✅ → **step 2 receiver requires the header** ← next, remembering it **auto-deploys on merge to `main`** → step 3 unbind, gated on `auth.verified_legacy_key` going quiet, which needs 0b *deployed* and a full traffic cycle observed. ⚠️ **Step 1 shipping does not narrow step 3's gate.** It stops the *sender* downgrading on a misconfiguration; it does not stop the receiver accepting a keyless signature from anything else, and it is only deployed code that counts — so `auth.verified_legacy_key` must still be observed quiet in production traffic, and step 1's own effect will not be visible there until this branch merges. That last dependency is the real schedule: nothing after step 2 can be verified until the instrumentation has been live long enough to be trusted. The dev receiver ([[CR02]] item 5) is **no longer on this critical path**; it is needed to *restore* the e2e job, not to ship step 2. Related: [[CR11]] row #7 (how it was found, and why provisioning `SIGNING_KEYS` into `dev` is not the fix), [[CR02]] item 5 (the dev receiver, now a prerequisite), [[CR01]] step 3 (the rotation this makes incomplete), [[W05]] (runbook — rewritten 2026-07-31), [[CR14]] (the receiver's retained versions publish whatever it binds, so step 3 also shrinks that exposure).
