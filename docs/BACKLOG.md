...
**Status:** Open — **step 0 done 2026-07-31 including its blocker; step 0b's instrumentation written and committed 2026-08-01 (`8fcae0b`, unpushed). Step 1 is next.** Step 0 found one automated caller signing without a key id (the CI e2e job) and that job has been **removed**, so `sender-worker` — signing `v2` — is the sole automated caller and nothing now depends on the keyless fallback. **Order, with what remains:** ~~0b instrument the receiver~~ ✅ (unpushed, and **pushing it deploys the receiver**) → **step 1 sender fail-closed** ← next, and cheap → step 2 receiver requires the header, remembering it **auto-deploys on merge to `main`** → step 3 unbind, gated on `auth.verified_legacy_key` going quiet, which needs 0b *deployed* and a full traffic cycle observed. That last dependency is the real schedule: nothing after step 2 can be verified until the instrumentation has been live long enough to be trusted. The dev receiver ([[CR02]] item 5) is **no longer on this critical path**; it is needed to *restore* the e2e job, not to ship step 2. Related: [[CR11]] row #7 (how it was found, and why provisioning `SIGNING_KEYS` into `dev` is not the fix), [[CR02]] item 5 (the dev receiver, now a prerequisite), [[CR01]] step 3 (the rotation this makes incomplete), [[W05]] (runbook — rewritten 2026-07-31), [[CR14]] (the receiver's retained versions publish whatever it binds, so step 3 also shrinks that exposure).
