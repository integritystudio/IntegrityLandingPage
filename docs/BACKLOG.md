...
3. ⚠️ **In progress (2026-07-29). Per-family state, verified by probe:**
   ...
   - ⚠️ **Supabase anon**: the legacy anon key was disabled at the project (the old anon JWT now gets `401`), but the replacement values landed in the wrong Doppler slots: `prd SUPABASE_ANON_KEY` holds the **legacy `service_role` JWT** (RLS-bypassing — must never sit in an anon slot; anon slots feed frontends), the new `sb_publishable_…` key sits in `dev SUPABASE_SERVICE_KEY`, and the four dashboard slots (`REACT_APP_`/`VITE_SUPABASE_ANON_KEY`, both configs) plus `dev SUPABASE_ANON_KEY` still hold the **dead** legacy anon key — dashboard builds pulling from Doppler will 401.
   - 🔴 **Supabase legacy `service_role` JWT is still enabled** — verified live 2026-07-29: an Oct-2025-issued legacy `service_role` JWT authenticates against PostgREST with full RLS bypass. It very likely sat in the leaked bundle, making [[CR24]]'s disable the single highest-value remaining rotation action.
   ...
   - ⚠️ **Auth0**: `AUTH0_CLI_SECRET` (M2M client → Management API, used by `/signup`) rotated 2026-07-29, validated via a `client_credentials` grant against the tenant, and re-bound to production `sender-worker` (healthy after). Doppler `dev` still holds the previous value. **`AUTH0_CLIENT_SECRET` (ROPC client, used by `/signin` and `/signup`'s sign-in step) remains unrotated.**
   - 🔴 **HMAC `SHARED_SECRET`**: untouched.
