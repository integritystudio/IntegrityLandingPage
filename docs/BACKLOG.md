...
3. ✅ **Done 2026-08-08 — `npm run dashboard:workers`** (`scripts/worker-dashboard.sh`), covering the provisioning path (`sender-worker` → `api-provisioning-receiver`) plus the rest of the production fleet, on Cloudflare Workers Analytics. Documented in [`docs/api-provisioning.md`](api-provisioning.md) § Monitoring Runbook → The dashboard, which also closes the half of step 5 that could not be written while no dashboard existed.
   ...
   >
   > So traces have not flushed to D1 for a week while metrics look healthy — a partial drain reads as a working pipeline from any check that samples one signal. Fixing it is `observability-toolkit`'s (raise `cpu_ms`, or chunk the flush so a run cannot starve a signal); this repo's dashboard reports it but cannot fix it. Historical detail from the original 2026-07-31 finding follows.
...
**Status:** Open — **all five steps are implemented; the only thing left is confirming step 4 fires after merge.** Step 3 closed 2026-08-08 (`npm run dashboard:workers`); its recorded blocker turned out to apply to only one of the two destinations the step offered — see the ⚠️ note under step 3. That closure also re-measured the `obtool-ingest` blocker and found it mis-stated: the cron fails ~100% of runs, not "~90% of invocations", and `org:…:traces` has not flushed to D1 since 2026-08-01. **That is now the only cross-repo item this entry is waiting on, and it belongs to `observability-toolkit`.** Step 1: instrumentation deployed and emitting on all four production Workers (2026-07-30). Step 2: signals defined and executable (2026-07-31, see above). Step 4: daily scheduled alert job added 2026-08-08 (`.github/workflows/worker-signals.yml` — GitHub job-failure email is the channel; Supabase creds enable SIGNAL 5 dead-letter depth) — ⚠️ **added, not armed: `schedule` workflows run from the default branch only, so it cannot fire until this branch merges to `main`, and no scheduled run has ever executed.** The check itself is verified live (all five signals, exit 0, `stripe-webhook` 1.00 subreqs/req); the *scheduling and notification* half is unproven. See the red note under [[CR20]]. Step 5: monitoring runbook added to `docs/api-provisioning.md` 2026-08-08, and extended the same day with the dashboard section that could not be written before step 3 existed. **Remaining: confirm step 4 actually fires post-merge** — that is the whole of it on this side.
