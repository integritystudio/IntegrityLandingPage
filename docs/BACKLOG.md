...
🔴 **CORRECTION, same day: CR20 was closed on HALF its stated gap and is REOPENED.** Its wording was that the *"scheduling **and** notification"* half was unproven. The test below proves **notification**. It does not prove **scheduling**, and the run list says so plainly: the repo has exactly **one** `worker-signals` run in its entire history, `2026-08-08T15:44:02Z`, event **`workflow_dispatch`** — the one deliberately triggered here. **Zero `schedule`-triggered runs have ever executed.** The `*/5` experiment ran ~15 minutes and produced nothing, which is consistent with GitHub scheduler latency but proves nothing either way.
...
> ✅ **Closed 2026-08-08. `CLOUDFLARE_D1_TOKEN` accepted and documented; six latent values fixed; the generalisable half built and mutation-proven.** Read [[W12]] before treating this as "environment isolation is done" — closing this item surfaced five more cross-environment values, one of which grants dev read of the entire production config.
>
...
Current reading: **170 shared, 110 byte-identical, 95 shared by design, 10 accepted, 5 known gaps ([[W12]]), 0 unclassified — exit 0.**
...
⚠️ **What closing this cost, and it is the honest headline: the sweep found five more, and [[W12]] now tracks them.** One is a `dp.st.prd.` Doppler service token in the `dev` config — a credential that reads the entire production config, i.e. the isolation boundary this item spent two days building. It was invisible to every check here until the polarity was inverted. **Nothing about this item's closure means the environments are isolated; it means the detector can finally see what is not.**
...
## W12: five more cross-environment values in Doppler `dev`, found by the sweep that closed [[W09]]

**Priority:** P2 (one of the five defeats the isolation boundary outright) | **Source:** session 2026-08-08, building [[W09]]'s full-config classifier
**Estimated:** ~1 hour, but three of the five are decisions rather than edits

**These exist because [[W09]] finally did what it kept asking for.** Its closing act was to stop hash-comparing a hand-maintained list and instead classify *every* name present in both configs. That pass immediately produced five names no list had ever mentioned. They are carved out rather than folded into W09 because W09's scope was the seven values it had characterised, and all seven are resolved — burying five new ones inside a closed item is how they become invisible again.

| Value | Why it is wrong | Read by |
|---|---|---|
| `IS_PROD_TOKEN` | prefix **`dp.st.prd.`** — a Doppler **service token scoped to the `prd` config**, sitting in `dev`. Anything with dev access can read the entire production config with it | nothing in either repo |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | `https://ingest.integritystudio.ai` — **production** ingest, under `dev` | nothing (the shipper reads the shell, not Doppler; the repo hits are documentation page copy) |
| `OBTOOL_INGEST_ROUTE` | `ingest.integritystudio.ai/*` — production route | nothing |
| `OBTOOL_API_KEY_INVENTORY_AI` | a **customer's** `obtk_` API key, byte-identical in both configs | nothing |
| `JWT_SECRET` | 44 chars, provenance unknown — plausibly a legacy Supabase JWT secret | nothing |

🔴 **`IS_PROD_TOKEN` is the one that matters, and it is a different *kind* of finding from anything W09 held.** Every other value in that item was a credential or endpoint that reached one production *resource* — a database, a tenant, a webhook. This one reaches **the credential store itself**. A `dev`-scoped process holding it can read every secret in `prd`, including the ones W09 spent two days making distinct. It makes the isolation boundary conditional on nobody using a token that is sitting right there.

**None of the five is read by any code in either repo**, which is why this is P2 and not P1 — and why the fix is cheap. It is also exactly the condition W09 warns is not a property to rely on: `PROVISION_WORKER_URL` was unread until a suite picked it up and started creating real users in production.

**Scope:**
1. **`IS_PROD_TOKEN` — delete from `dev`, do not repoint.** There is no dev-equivalent of a prd-scoped service token; the name describes a production credential. Confirm nothing outside these two repos consumes it first (it is unread *here*, which is not the same as unused — a local script or another machine could).
2. **`OTEL_EXPORTER_OTLP_ENDPOINT` / `OBTOOL_INGEST_ROUTE` — repoint at the dev ingest.** `obtool-ingest-dev` exists and is deployed. Left as-is, any future `doppler run -c dev` process ships telemetry into production. ⚠️ Check the interaction with the user-level `~/.claude` config first: the detached shipper reads `OTEL_EXPORTER_OTLP_ENDPOINT` from the **shell**, exported unconditionally from `zshrc`, so the Doppler slot and the shell export are two different sources for one variable — changing one does not change the other, and W09's "sixth instance" was precisely a mismatch between an OTLP endpoint and the credential shipped with it.
3. **`OBTOOL_API_KEY_INVENTORY_AI` — decide whether a customer key belongs in this project at all.** If it is needed for a dev-side integration test, it should be a dev-org key against `obtool-api-dev`, not the live customer's.
4. **`JWT_SECRET` — triage.** Establish what issued it before deciding; if it is the legacy Supabase JWT secret, it is the same dead-slot case as `SUPABASE_JWT_SECRET` (removed from the code 2026-07-31) and should be deleted rather than rotated.
5. As each is resolved, move it out of `KNOWN_GAP` in `scripts/check-env-isolation.sh`. **The check passes today only because these five are named there**, each with this item's id — that is a deliberate baseline, not a clean bill of health, and the table prints on every run so it cannot be mistaken for one.

⚠️ **Do not "fix" these by adding them to `SHARED_BY_DESIGN`.** That allowlist is for values with no dev/prd notion — third-party keys, account identifiers, business constants. Moving a real gap into it would convert a printed defect into a silent pass, which is the exact failure this whole detector exists to prevent.

**Files:** Doppler `integrity-studio` `dev` config (no repo change for steps 1–4); `scripts/check-env-isolation.sh` (`KNOWN_GAP` map) for step 5.

**Status:** Open — all five identified, classified and printed on every run; none fixed. Steps 1 and 2 are mechanical once step 2's shell-vs-Doppler interaction is confirmed; steps 3 and 4 need a decision about what the value is for.
