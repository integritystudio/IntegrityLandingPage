...
| [CR30](#cr30) | P1 | ✅ **resolved 2026-08-03** — CI guard written, **never executed** | **The migration ledger could not rebuild the schema — now it can, proven by replay onto an empty database.** Final parity: 24/24 tables+views, 255/255 columns, 3/3 enums. Five new migrations; three separate ordering defects in the *existing* ledger were only findable by replaying. Gap was 10 tables, 3 enums, 2 columns on a ledger-managed table, 1 view — 43% of the schema was unversioned. Production untouched (read-only queries; all new files idempotent). **CI guard written 2026-08-03** (`migration-replay-check.yml` → `check-migration-replay.sh`: full local stack, `db reset`, schema assertions incl. the view; mutation-tested assertion SQL) — but it has **never run**: Docker is absent locally, and `gh` 404s the workflow on the default branch because every file it needs is in the 36 unpushed commits. It triggers only on `main` (`push`/`pull_request` `branches: [main]`), so **pushing this branch does not run it** — the PR into `main` is the first execution. ⚠️ The summary here read "proven on its first CI run" until 2026-08-03: the body's pending gate ("the first CI run is the real proof") was compressed into a completed one, which is how an unexecuted guard came to read as a verified one. The drift check compares against production and cannot catch this class |
...
| [CR32](#cr32) | P3 | 🔴 open — owner decision + DNS | Auth0 **custom domain** (login runs on `dev-…auth0.com`). Carved from CR25 item 4. **NOT plan-gated** (POST 400, not 403) — blocked on a hostname choice (`auth.integritystudio.ai`?) + DNS (settable here) + tenant permanence |
...
| [CR34](#cr34) | P2 | 🔴 open — scriptable, verify login | Strip Auth0 **`implicit` + ROPC** grants (SPA + `AUTH0_MANAGER`). Carved from CR25 items 7–8. Minutes by API, but must verify `sender-worker`'s `password-realm` `/signin` survives; `My App`'s ROPC likely stays until the client gets a refresh flow |
...
**Status:** ✅ **Fixed in code 2026-08-03 — committed on `fix/active-subscription-id`, unpushed, and therefore not deployed.** The fragment is gone from `provision_page.dart`. ~~Full fix (postMessage / exchange code) requires a coordinated change in the dashboard app.~~ It did not — see the re-measurement below. *(Earlier: partially done 2026-07-26, commit ~~d632263~~ **`1c83136`** — misleading comment corrected; see the SHA note at the end of this entry.)*
...
**Status:** ✅ **Resolved 2026-08-03 — the ledger now rebuilds production's schema from an empty database, verified by replay.** Final parity against production: **24/24 tables+views, 255/255 columns, 3/3 enums, zero missing, zero extra.** The CI guard (scope step 4) is written and statically verified 2026-08-03 — `migration-replay-check.yml` + `check-migration-replay.sh`; it has **never executed** — Docker is absent on this machine, and the workflow is not on the default branch (`gh` 404), so its first run is the PR into `main`. No production change was made — every query against production was read-only, and all five new migrations are idempotent no-ops there.
...
| Worker | Secrets | Repo |
|---|---|---|
| `obtool-ingest` | 1 | `observability-toolkit` |
| `obs-toolkit-quality-metrics-api` | 2 | `observability-toolkit/dashboard` |
| `tcad-token-refresh` | 1 | **`tcad-scraper`** — a third repo |
| `quality-metrics-api` | 2 | **no config found** — likely the dashboard's former name, orphaned |
| `integrity-studio-cookie-manager-dev` | 1 | **no config found** — orphaned |

**None of the four locatable configs sets `preview_urls`** — including `api-provisioning-receiver/wrangler.toml`. That is the same default-true trap step 2 caught in this repo's `api-gateway` and `stripe-webhook` configs, unfixed in two other repos, which means **an API-level flip on the receiver will not survive its next deploy.** `tcad-api` holds 7 secrets but has `workers.dev` disabled outright, so it is not exposed.
...
3. Ask the `observability-toolkit` owner to do the same for `api-provisioning-receiver` (~~**10**~~ **8** secrets live 2026-08-03, not deployable from here). ✅ **Now tracked on the owning side too — `observability-toolkit/docs/BACKLOG.md`, `PREVIEW-URLS`** — which it was not before: that repo tracked none of this, so this step had never actually landed anywhere its owner would see it. **Re-quantified 2026-08-03: ~~30 of its 30 code-upload versions~~ 36 live of 89 retained, oldest 2026-03-20**, and two clauses here were wrong. "30 of 30" undercounted because the retained set has grown; and ~~"every one of the 30 serves with the current secret set, including the `SIGNING_KEYS` bound at 01:29 that same morning"~~ is **false** — each version serves the bindings *it* was uploaded with, so the 2026-03-20 version holds no secrets and the pre-rotation versions hold pre-rotation values. See the mechanism correction at the top and the era classification above. Also fix the config, not just the live setting: `services/api-provisioning-receiver/wrangler.toml` has `workers_dev = true` and **no `preview_urls` key**, so an API flip there is undone by its next deploy — and that repo auto-deploys on push to `main`.
...
| Worker | Secrets | Fix |
|---|---|---|
| `api-provisioning-receiver` | ~~**10**~~ **8** | **Cross-repo** — needs the `observability-toolkit` owner (step 3, now filed there as `PREVIEW-URLS`). ~~**30 superseded versions reachable**~~ **36 live of 89 retained**, back to 2026-03-20 |
...
4. ➡️ **Carved out to [[CR32]] (2026-08-03)** — custom domain. NOT plan-gated (POST 400, not 403); blocked on an owner hostname decision + DNS. See CR32.
...
### CR31: the published API docs advertise four URLs that resolve to nothing, and `api-gateway` has no hostname
...
**Measured, not assumed (2026-08-03):** this is **not plan-gated**, contra CR25's original wording. `POST /api/v2/custom-domains` with an empty body returns **400 payload-validation** (missing `type`/`domain`), not the **403** a feature-gated endpoint returns — so the plan allows a custom domain. What blocks it is threefold and none of it is a config toggle:

1. **Owner decision — the hostname.** e.g. `auth.integritystudio.ai` or `login.integritystudio.ai`. Not mine to choose.
2. **DNS.** Auth0 returns CNAME/TXT verification records that must be added to the zone. DNS *is* reachable from here (porkbun / Cloudflare MCP), so once the hostname is chosen this is completable end to end.
3. **Permanence.** A custom domain makes the tenant permanent — moving or removing it later invalidates existing sessions and bookmarks. That is why it should not be created speculatively.

**Sequence once the hostname is decided:** `POST /custom-domains {type:"auth0_managed_certs", domain:"<host>"}` → read back the verification records → add them to DNS → `POST /custom-domains/{id}/verify` → update the app's allowed callback/logout URLs and `AUTH0_DOMAIN` consumers if the login URL is user-facing.

**Status:** Open — blocked on the hostname decision (owner). Everything after that is scriptable from here.
...
**Status:** Open — unblocked and scriptable, but gated on a per-client login-path verification, not a blanket strip.
