...
## Code Review 2026-07-26 → 2026-07-27 (CR01–CR29)

Started as the open remainder of the 8-area codebase review; CR11–CR15 were found afterwards while deploying and auditing the workers, CR16 while reading the deployed `obtool-*` scripts to settle CR13, CR22–CR23 as follow-ups to the billing-portal auth change, CR26 while fixing the reported dashboard CORS failure — which turned out to sit on top of two deeper auth defects — and CR29 while diagnosing CR11 row #7, where the shared secret was the symptom and the unrotatable legacy key path was the actual defect. Fixed work lives in [`changelog/1.3/CHANGELOG.md`](changelog/1.3/CHANGELOG.md); the review's method, provenance, and 3 refuted claims are in [`CODE_REVIEW.md`](../CODE_REVIEW.md).
...
| [CR02](#cr02) | P2 | ✅ mostly | Dev/prod split done and verified live. ~~only the dev receiver remains~~ **2026-08-02:** it is a *config-layer* split with no credential behind it (new step 8 — `deploy` and `deploy:prd` share one `CLOUDFLARE_WORKER_TOKEN`) and no data isolation behind it ([[CR11]]). The dev receiver (item 5) is **blocked behind CR11 step 1** — standing one up against the shared Supabase project would write real orgs and mint real keys in production, worse than today's fail-closed state |
...
**Estimated:** 3–5 hours → ~1 hour remaining, plus a cross-repo change — ~~and it is *not* ready to start~~ **corrected 2026-08-02: item 5 is blocked behind [[CR11]] step 1, which this item never recorded. See item 5.**
...
5. **Remaining — deploy a dev receiver. 🔴 Blocked behind [[CR11]] step 1 — do not start it first.** `sender-worker-dev` still binds `RECEIVER` to the production `api-provisioning-receiver` (a **service binding**, `[[env.dev.services]]`, naming the same Worker as the top-level block); no dev receiver exists, and it lives in the `observability-toolkit` repo. Cross-repo.

   **Why the order matters, recorded 2026-08-02 because this item read as ready-to-do for a week.** The receiver's `SUPABASE_URL` is a plaintext var in its `wrangler.toml` pointing at the production project, and `dev`/`prd` share that project ([[CR11]]). A dev receiver stood up before CR11 step 1 would run `ensureTeamOrg` / `addOrgMember` / `grantDashboardAccess` and mint real keys via `api-keys-create` **against the production database, successfully** — strictly worse than today, where `sender-worker-dev` has zero secrets bound and simply fails. Today's safety is *credential absence*, not design; nothing enforces it.
...
8. **Scope the dev deploy token** (added 2026-08-02; free, unblocked, independent of item 5). This item's whole guarantee — `npm run deploy` cannot overwrite production — is enforced at the *config* layer by `deploy-environments.test.ts` and at the *credential* layer by nothing: `deploy` and `deploy:prd` both authenticate with the same `CLOUDFLARE_WORKER_TOKEN` from Doppler `prd`, so only argument order stops a dev deploy reaching a production Worker. Mint a token scoped to the `*-dev` Workers, put it in Doppler `dev`, have `deploy` read that one. Cross-referenced as [[CR11]] step 8 so it lands with the rest of the isolation work; recorded here because it is *this* item's guarantee that is thinner than it looks.
...
**Status:** Done and verified live (2026-07-27) — `npm run deploy` can no longer overwrite a production worker, enforced by `workers/lib/deploy-environments.test.ts` (31 tests, mutation-verified) and confirmed by an actual deploy. ~~Only item 5 remains (dev receiver, cross-repo).~~ **Restated 2026-08-02 — two items remain and they are not equally available.** Item 8 (scoped dev deploy token) is free and unblocked and should be done next; item 5 (dev receiver) is **blocked behind [[CR11]] step 1** and would make things worse if started first, though the part of it that removes the `[env.dev]` `RECEIVER` binding is unblocked and worth doing on its own. The guarantee this item already delivers is real but narrower than "dev/prod separation" suggests: it is a *config-layer* guarantee with, as of today, no credential behind it (item 8) and no data isolation behind it at all ([[CR11]]).
...
**Estimated:** ~2 hours of mechanical work, behind two manual provisioning actions of ~15 minutes each
...
**Detector:** `npm run check:env-isolation` — compares credential hashes between the two configs, prints no secret material, exits non-zero while they are shared. ~~**Currently fails 3 of 13**~~ — **fails 5 of 13, measured 2026-07-31** (was 10 of 10 before the 2026-07-29 work; the row count grew when Stripe coverage was added). A green run is the definition of done — but see the 2026-07-29 update below: `AUTH0_DOMAIN` is not reachable by API at all, so **2 of 3 is the API floor**.
...
3. **[[CR01]]'s claim that `dev` holds no working RLS-bypassing Supabase credential is false today.** That claim rested on `dev`'s `SUPABASE_SERVICE_ROLE_KEY` holding the dead pre-rotation value — but the slot is gone, and the live key sits in the shared `SUPABASE_PROVISIONING_KEY`. **Probed, not inferred:** `dev`'s copy against `https://cfrbahzzklwrnmbtqojl.supabase.co/rest/v1/organizations` returns **HTTP 200**. Selecting `--config dev` therefore still yields full read/write on the production database. This is the single most consequential thing on this page that the page got wrong.
...
8. **Scope the dev deploy credential** (added 2026-08-02; independent of steps 1–2, free, do it any time). `deploy` and `deploy:prd` both authenticate with the same `CLOUDFLARE_WORKER_TOKEN` from Doppler `prd`, so the dev/prod split enforced by [[CR02]]'s `deploy-environments.test.ts` is a *convention* at the config layer with no credential behind it — only argument order stops a dev deploy reaching a production Worker. Mint a token scoped to the `*-dev` Workers, put it in Doppler `dev`, and have `deploy` read that one. The stronger form Cloudflare documents is separate accounts with wrangler profiles pinned per directory; that is a bigger change and is **not** proposed here — the scoped token is the cheap majority of the benefit.
9. **Give dev its own Stripe sandbox** (added 2026-08-02; independent of steps 1–2, free). Stripe's guidance is a distinct sandbox per environment — staging, CI and local dev each their own — so test data cannot collide. `stripe-webhook-dev` already holds sandbox `sk_test_`/webhook-secret values and is the model to follow; the rule to hold is that **`dev` never receives a copy of a `*_live_` key**, restricted or not. The live `rk_live_…` now in `prd` is itself correct practice — restricted keys are recommended for server-side use — which is exactly why copying it would be the tempting mistake.
...
**Consumers of this item, so its priority is not judged in isolation:** [[CR02]] item 5 (the dev receiver) depends on step 1 but **records no dependency on it at all**, and its "~1 hour remaining" reads as ready-to-do. It is not: a dev receiver deployed against the shared project would run `ensureTeamOrg` / `addOrgMember` / `grantDashboardAccess` and mint real keys via `api-keys-create` **in the production database, successfully** — strictly worse than today, where `sender-worker-dev` has zero secrets and simply fails. Separately, the `observability-toolkit` e2e suite was removed from CI on 2026-07-31 because `--config dev` points at the **production** receiver, and step 7 is what lets it come back. Both unblock behind step 1.
...
- **Rows #4–#6 — ✅ done 2026-07-29, taking the detector to 3 of 13 that day** (5 of 13 now; these three rows have held — see the 2026-07-31 update above). A separate user store plus dev-only clients, all via the Management API:
...
**Sequenced target:** ~~#7 gets 7→6~~ and ~~the Auth0 dev connection + dev clients get 6→3~~ — both done 2026-07-29, reaching 3 of 13 that day. ⚠️ **Not the current state: 5 of 13 as of 2026-07-31** — #7 regressed and the `SUPABASE_SERVICE_ROLE_KEY` row is now a phantom failure against a deleted slot (see the 2026-07-31 update above). The Auth0 half of this has held. What remains: a dev Supabase project takes 3→1 (~~spend/quota decision~~ **free — corrected 2026-08-02, see *Blockers and cost*; the only unknown left is that `POST /v1/projects` has no dry-run, so the quota is confirmed by attempting it**), and the final row needs ~~one Dashboard visit to create a second Auth0 tenant~~ **one Dashboard visit to mint an M2M credential for the second tenant that already exists** (`dev-njjmghdzm23uy0p7`, live — corrected 2026-08-02; this line had contradicted the 2026-07-29 update above since the day that update was written) — after which the connection/client work above should be redone inside it, and the `dev-users` connection in the production tenant retired. Neither remaining row is gated on money.
...
4. **Add a CI guard that replays the full ledger onto an empty database**, so "the migrations build the schema" becomes a tested property rather than an assumption. The existing drift check cannot catch this class by construction — it only ever compares against a database that already works.

**Status:** ✅ **Resolved 2026-08-03 — the ledger now rebuilds production's schema from an empty database, verified by replay.** Final parity against production: **24/24 tables+views, 255/255 columns, 3/3 enums, zero missing, zero extra.** One item remains open and is tracked as scope step 4: the CI guard that replays onto an empty DB, without which this regresses silently. No production change was made — every query against production was read-only, and all five new migrations are idempotent no-ops there.
