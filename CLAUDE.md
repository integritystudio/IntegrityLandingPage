...
- **CR11**: Doppler `dev` holds the same Supabase project and Auth0 tenant as `prd`. `--config dev` is not a safety boundary. Detector: `npm run check:env-isolation` — **6 of 13 failing** as of 2026-07-29 (was 10/10; it now covers 3 Stripe rows too). The 6 are `SUPABASE_URL` + `SUPABASE_ANON_KEY` (one project) and `AUTH0_DOMAIN`/`CLIENT_ID`/`CLIENT_SECRET`/`CLI_ID` (one tenant). **The API floor is 1**: Auth0 tenant creation has no endpoint and the M2M token lacks `create:tenants`, so `AUTH0_DOMAIN` is Dashboard-only. See BACKLOG.md CR11 for the per-row feasibility table
