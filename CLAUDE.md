...
**Not deployable, and now structurally so.** `receiver-worker` is a local stub / test double. Its `deploy`/`deploy:prd` scripts were **removed 2026-09-24** — they would have recreated a Worker that was deleted from Cloudflare on 2026-06-26, under its old production name, binding nothing and returning mock responses. `deploy-environments.test.ts` asserts their absence (`STUB_WORKERS`), and `scripts/deploy-provisioning-workers.sh` — the one path that called `wrangler deploy` there directly, bypassing the package scripts — was deleted in the same pass. `bootstrap-worker` was deleted 2026-07-31; `POST /bootstrap` is a route on `api-gateway` (verified live: **401**, not 404, so the route is mounted and auth-gated).
