...
**Two Workers were deliberately NOT deployed.** `bootstrap-worker` has **no production deployment at all**, so `deploy:prd` would *create* a new publicly-callable Worker rather than update one, and it would come up with zero secrets bound. ⚠️ **Its absence is not cost-free, though** — corrected 2026-07-30: the shipped Flutter app calls `POST {api-gateway}/bootstrap`, a route `api-gateway` does not serve, because the handler lives in this undeployed Worker. The screen shown immediately after signup has therefore never loaded. Tracked as BACKLOG.md CR26, which is open — standing it up is a real fix, not just a new surface. `receiver-worker` is a local stub / test double (`name = "receiver-worker"`), so its `deploy:prd` would likewise create a production Worker that nothing binds to and that returns mock responses. Neither should be run without a decision to stand that service up.
