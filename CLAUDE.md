...
**✅ Doppler `dev` is now data-isolated from `prd` (2026-08-03) — `npm run check:env-isolation` PASSES.** `--config dev` reaches a separate Supabase project (`tumhmtshahktumhqqamk`) and a separate Auth0 tenant (`dev-njjmghdzm23uy0p7`); the dev provisioning key is refused by production (401). Stripe was already isolated (separate sandbox account). ⚠️ **Two things this does NOT mean.** (1) The deployed `*-dev` workers still hold **zero secrets** — repointing Doppler changes what *local/CI* processes read, not the deployed workers, which is a **separate** step (`wrangler secret put --env dev`) and the intended next action now that the detector is green. (2) The dev database is **empty** — a repointed `--config dev` reads nothing until it is seeded. **Do not push `dev` secrets into the workers before confirming the detector still passes**, and never push `prd` values into a `*-dev` worker. Tracked as BACKLOG.md CR11. *(Historical: before 2026-08-03 both configs shared one Supabase project and Auth0 tenant, so `--config dev` was not a safety boundary.)*

⚠️ **The dev/prod deploy split has no credential behind it.** `deploy` and `deploy:prd` both authenticate with the same `CLOUDFLARE_WORKER_TOKEN` from Doppler **`prd`**, so what stops `npm run deploy` reaching a production Worker is `--env dev` in the script plus `workers/lib/deploy-environments.test.ts` asserting it — a *config-layer* guarantee, not an authorization one. Cloudflare's stronger documented form is separate accounts with wrangler profiles pinned per directory so a command **cannot** reach the wrong account; the cheap majority of that is a dev-scoped token in Doppler `dev`. BACKLOG.md CR11 step 8 / CR02 step 8.
