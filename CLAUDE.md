...
**Test Status**: ✅ **1,212 worker tests** measured 2026-08-02 on the working tree (`lib` 515, contact-form 81, api-gateway 208, sender-worker 203, receiver-worker 33, stripe-webhook 172), and **3,017 Flutter** (2026-07-31). Counts in this file have gone stale by ~100 before — if you are reconciling one, run the suite rather than trusting the line. Coverage ~94%. `npm run lint:workers` **is** the worker linter (`tsc --noEmit` × the 6 packages; there is no ESLint under `workers/`, and plain `npm run lint` is `flutter analyze`) — zero errors as of 2026-08-02. ⚠️ **A green `lint:workers` says nothing about test files in `sender-worker`** — its `tsconfig.json` excludes `src/**/*.test.ts`, `*.e2e.test.ts`, `*.live.test.ts` and `src/test-helpers/`, which is **111 type errors `tsc` never reports** (measured 2026-08-02; 116 at `origin/main`, so the number moves and is not a regression signal on its own). The other five packages do type-check their tests. This is not cosmetic: making a field non-optional in `Env` silently invalidates every partial `Env` literal in those files, and neither `tsc` nor `vitest` says a word — that is exactly how CR29 step 2 broke three fixtures. Check with a throwaway config that clears `exclude` before changing an `Env` shape. Opt-in suites: `sender-worker` `test:e2e` **49/49** (2026-08-02), `test:live` 9 passed / 3 skipped and `stripe-webhook` `test:live` 5/5 (both 2026-07-29)
...
- **CR29**: 🔴 **the HMAC key rotation retires nothing — fixed in code 2026-08-02, and nothing is deployed, so every statement here is still true of production.** `SIGNING_KEYS`/`ACTIVE_KEY_ID=v2` is provisioned on both sides and works — but the live receiver resolves an **absent** `x-key-id` to the legacy `SHARED_SECRET`, a credential with no key id and therefore no rotation handle. Measured against production `POST /inbox` with controls: `v2` + key id → 200, `SHARED_SECRET` + **no** key id → **200**, garbage → 401. So removing a key from `SIGNING_KEYS` revokes nothing, CR01's HMAC rotation is incomplete, and anything holding `SHARED_SECRET` (incl. Doppler `dev`) can forge provisioning events by omitting one header.
  ...
  - **Two toolkit paths are type-checked by nothing.** The receiver's `tsconfig.json` is `include: ["src/"]` and the root's is `include: ['src/**/*']`, so `services/e2e/` and `services/api-provisioning-receiver/scripts/` are covered only by `eslint.config.cjs` (lines 181–190) — run `npx eslint --config eslint.config.cjs <file>` after editing either. A green `tsc` says nothing about them.
