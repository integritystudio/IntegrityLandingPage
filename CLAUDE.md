...
**Supabase** (migrations are the source of truth for schema — ✅ **true since 2026-08-03, and it was not before**: the ledger could not rebuild the schema at all. 10 tables, 3 enums, 2 columns on a ledger-managed table and 1 view — 43% of `public` — existed only in production. `migration list` said "zero out of sync" the whole time because it compares against **production**, which already had every object. **A migration set is only proven by replaying it onto an empty database**; that had never been done. Five baseline migrations now close it, verified at 24/24 tables+views and 255/255 columns. The CI guard is `migration-replay-check.yml` → `scripts/check-migration-replay.sh` — full local stack + `db reset` + schema assertions; written 2026-08-03 and **never executed** — Docker is absent on this machine, and `gh` 404s the workflow on the default branch because it is still unpushed. Both triggers filter `branches: [main]`, so **pushing a feature branch runs nothing**; the PR into `main` is the first execution. *This line read "proven on its first CI run" until 2026-08-03 — a pending gate compressed into a completed one, so an unexecuted guard read as a verified one. `gh run list --workflow=migration-replay-check.yml` is the check that settles it.* A second dev project, `tumhmtshahktumhqqamk` / `integritystudio-dev`, exists for cloud-side replay.)
```bash
...
**Phase**: Codebase review remediation + worker deploy/settings audit + database/secret remediation + credential rotation — CR01–CR31, status table in [docs/BACKLOG.md](docs/BACKLOG.md)
...
⚠️ **`sender-worker` deploys via CI on merge to `main`, so anything unmerged is not live *unless deployed manually*.** CR29 (steps 1+2) was deployed to production by **manual `deploy:prd`** 2026-08-03 — so production runs it while it is still only on this branch (`4bd0901` step 1, `4ce2608` step 2), **ahead of `origin/main`**. Consequence to remember: a CI redeploy of `sender-worker` from `main` *before this branch merges* would silently roll the sender back to the pre-CR29 fallback (defense-in-depth only — the receiver-side forgery fix is on toolkit `main` and durable, and prod signs `v2` regardless). **Merge the branch to make the sender fix durable.** The reverse hazard also holds generally — pushing a stale `main` deploys older code over what is live.
...
### Known Issues
Status table in [docs/BACKLOG.md](docs/BACKLOG.md#code-review-2026-07-26--2026-07-27-cr01cr31), CR01–CR31. What remains open needs a credential decision, a spend decision, an answer about intent, or a production deploy.

**P1**
- **CR29**: ✅ **the forgery path is CLOSED in production (deployed + verified 2026-08-03).** Sender fail-closed (`sender-worker` `d6764d99`) + receiver requires `x-key-id` (`api-provisioning-receiver` `e964852d`), both via manual `deploy:prd`. Re-measured against production `POST /inbox` with curl + controls: `SHARED_SECRET` + **no** key id → **401** (was **200** — the fix), `v2` + key id → signature passes (400 downstream on a fake body), garbage → 401; legit `/signin`→`/send` → `ok:true`. ⚠️ **Two probe traps** (both caught by the positive control): Python-urllib gets the `1010` bot-block on `workers.dev` (use curl); a *seconds* timestamp fails `validateTimestamp` against the sender's `Date.now()` *milliseconds* before signature. ✅ **Step 3 DONE 2026-08-03** — `SHARED_SECRET` unbound from both workers, its Doppler `prd` slot deleted, dropped from `KEY_ROTATION_DATES` ({v2}), and the receiver `Env` field made optional. Post-unbind prod re-verified (keyless→401, v2 passes, `/signin`→`/send`→`ok:true`). The traffic-observation gate was skipped by decision — the receiver already 401s keyless, so an inert unbind can't break any caller. **Durability:** the security fix is receiver-side (toolkit `main`, durable); the sender fail-closed change is on `fix/active-subscription-id` — merge to make step 1 durable (a sender regression is defense-in-depth only; prod signs `v2` regardless).
...
- **CR31**: 🔴 **the published API docs advertise four URLs that resolve to nothing, and `api-gateway` has no hostname.** Full inventory in [docs/api-routing.md](docs/api-routing.md) (measured 2026-08-03 — re-measure with the commands there rather than trusting this paragraph). `api.integritystudio.ai/*` → **`obtool-api`**, the observability read API; `api-gateway` — account, billing, ingest, `/bootstrap` — has **no zone route at all** and is served to customers at `api-gateway.alyshia-b38.workers.dev`, which is also the Flutter default that `ci.yml` ships. **Four** customer-visible defects, needing no decision: `docs_quickstart_page.dart:515` advertises `GET /v1/health` which **401s** (health is at `/health`; `obtool-api` mounts `authMiddleware` on `/v1/*` and it catches the path first), `docs_alerts_page.dart:217` advertises `POST /v1/alerts` which exists on **neither** worker, and `docs_api_page.dart:119` (`sandbox-api.integritystudio.ai`) plus `docs_index_page.dart:498` (`status.integritystudio.ai`, the docs landing page's Status quick-link) are both **NXDOMAIN**. **Do not repoint the wildcard to `api-gateway`** — the two route tables are disjoint apart from `/health`, so that 404s all thirteen `obtool-api` routes including `/v1/traces`, the one advertised endpoint that works. The fix is four narrower patterns (`/v1/me`, `/v1/orgs*` — api-keys are nested under it — `/v1/ingest/*`, `/bootstrap`) with the wildcard left as fallback, needing no code change on either worker; it does mean putting `routes` back in `workers/api-gateway/wrangler.toml`, **top level only with an explicit `routes = []` under `[env.dev]`**, which is CR13's 2026-07-27 incident. ⚠️ **Three traps, all of which produced wrong readings while this was measured:** `curl` defaults to **GET**, so `api-gateway`'s POST-only routes read as 404/missing; a **401 proves the middleware ran, not that the route exists** — every path under `obtool-api`'s `/v1/*`, real or invented, returns 401; and **a bare-substring grep merges subdomains** — `api.integritystudio.ai` is contained in `sandbox-api.integritystudio.ai`, so the first version of the advertised-URL check emitted one host for two and hid the `status.` dead link entirely. Anchor on `https?://`. Read the dispatch table from source; probe only what is live
- **CR13**: ⚠️ partial — step 1 done (the `routes` key is gone from `workers/api-gateway/wrangler.toml`) and **proven in practice 2026-07-30**: a real `deploy:prd` ran and the zone routes were unchanged afterwards, so `deploy:prd` will not capture `obtool-api`'s traffic. The four months of undeployed fixes have shipped. The hostname-topology decision — what URL customers should use — is still open (BACKLOG.md CR13 steps 3–5), but it is now a **decision rather than an investigation**: CR31 measured both surfaces and recommends the path-split above
...
- **CR04**: ✅ **fixed in code 2026-08-03, committed on `fix/active-subscription-id` and unpushed, so not deployed** — the fragment handoff is deleted; `_goToDashboard` (`lib/pages/provision_page.dart:86-92`) opens the dashboard with no token. Ships on merge to `main` (`ci.yml` `deploy-cloudflare` → Pages `integritystudio-ai`, gated on `refs/heads/main` + `push`). `access_token` now appears nowhere in `lib/`, `test/` or `e2e/`. ~~cross-repo fix needed~~ **it was never cross-repo; re-measured 2026-08-03.** The receiving app at `integritystudio.dev` is `integritystudio/quality-metrics-dashboard` on GitHub Pages, and it **never reads the fragment**: zero `location.hash` reads in the deployed bundle, its `wouter` router sources location from `location.search`, and every `access_token` string in it belongs to `@auth0/auth0-spa-js` internals. It runs its own Auth0 SPA login (`loginWithRedirect`, refresh tokens, tenant `dev-68gg87ow4mg4kzyo`, audience `https://api.integritystudio.dev` — which **matches** what this repo mints, so the handoff is unread rather than unusable). So the `postMessage` / exchange-code design in CR04's scope is unnecessary and no coordination is required: **delete the fragment**, a one-line unilateral change. Two things that keep it worth doing — the token still lands in that origin's address bar and history, and **that origin cannot be hardened** (GitHub Pages sets no headers, so no CSP, no `Referrer-Policy`, no HSTS, and no `<meta http-equiv>` CSP either). Verified two independent ways (GitHub code search on the source repo + grep of all five fetched asset chunks, with **no lazy chunk names**, so the bundle is the whole app)
