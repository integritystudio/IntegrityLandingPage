...
- **CR01**: ⚠️ partial — history scrubbed + force-pushed 2026-07-29. Rotation underway: Stripe `rk_live_` rotated, validated, re-bound to `api-gateway` + `sender-worker` (old key still needs Dashboard revocation). **Supabase values are mis-slotted in Doppler**: `prd SUPABASE_ANON_KEY` holds the legacy `service_role` JWT, `SUPABASE_JWT_SECRET` holds a UUID that verifies nothing (the real JWT secret now lives only in `api-gateway`'s binding — **do not re-bind it from Doppler**), and the legacy `service_role` key is still enabled → [[CR24]] is urgent. `AUTH0_CLI_SECRET` rotated + re-bound to `sender-worker`; `AUTH0_CLIENT_SECRET` (ROPC) + HMAC `SHARED_SECRET` still pending. Full state: BACKLOG.md CR01 step 3
